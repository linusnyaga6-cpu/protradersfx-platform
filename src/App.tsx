...
import { AuthContext, LegacyAuthProvider, useAppAuth, type AppAuthState } from '@/auth';
import {
  ArrowUpRight,
  BarChart3,
  Bot,
...
const authReturnUrl = `${basePath || ''}/?view=dashboard`;
...
function Home() {
  ...
  function handleReview(event: FormEvent<HTMLFormElement>) {
    ...
    setReviewState(`${contractSide} proposal ready for review · ${stake || '0'} USD · ${duration}`);
  }
          ...
          <form className="trade-ticket" onSubmit={handleReview}>
            <div className="trade-ticket-header">
             ...
             <div className="ticket-body manual-contract-body">
              ...
              {reviewState && <p className="review-state" role="status">{reviewState}. This is a review only; no trade was placed.</p>}
            </div>
          </form>
        </div>
        </>
    ...
    <main className="public-landing">
      ...
      <header className="public-landing-header">
        <a href={basePath || '/'} className="public-landing-brand"><span><BarChart3 size={18} /></span><strong>PROTRADERS <b>FX</b><small>POWERED BY DERIV</small></strong></a>
         <div className="public-landing-actions"><a className="public-login" href={signInHref}>Log in</a><a className="public-signup" href={signUpHref}>Get started</a></div>
      </header>
      <section className="public-hero public-hero-minimal">
        <div className="public-hero-copy">
          <h1>Trade like a <span>pro.</span></h1>
           <div className="public-hero-actions"><a className="public-hero-primary" href={signInHref}>Enter terminal <ArrowUpRight size={15} /></a><a className="public-hero-secondary" href={signUpHref}>Create account <Zap size={14} /></a></div>
        </div>
...
function BulkTraderView({ activeMarket, setActiveMarket, marketQuotes }: { activeMarket: string; setActiveMarket: (market: string) => void; marketQuotes: Record<string, MarketQuote> }) {
  const activeDefinition = getMarketDefinition(activeMarket);
  const activeQuote = marketQuotes[activeDefinition.symbol];
  ...
  return (
    <section className="bulk-trader-view">
      <form onSubmit={handleBulkReview}>
        ...
        <div className="bulk-bottom-row"><button type="button" className={`bulk-auto-button ${autoTrader ? 'is-on' : ''}`} onClick={() => setAutoTrader(!autoTrader)}><Settings2 size={12} /> {autoTrader ? 'Review auto mode on' : 'Review auto mode'}</button>{reviewState && <span role="status">{reviewState}. Review only; no live trade was placed.</span>}</div>
      </form>
    </section>
  );
}
...
function ClerkAuthBridge({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AppAuthState>(() => ({
    ...
    user: user ? {
      label: user.username ?? user.primaryEmailAddress?.emailAddress ?? user.id,
      email: user.primaryEmailAddress?.emailAddress,
    } : null,
    activeMode: 'demo',
    balance: null,
    currency: 'USD',
    signInHref: `${basePath}/sign-in`,
    ...
    <QueryClientProvider client={queryClient}>
      <TooltipProvider>
        <WouterRouter base={basePath}>
          {clerkPubKey ? <ClerkRoutes /> : <LegacyRoutes />}
        </WouterRouter>
        <Toaster />
      </TooltipProvider>
    </QueryClientProvider>
